...
def pobierz_dane_bi(rok, api_key=None):
    ...
    for nazwa_kolumny, (zmienna_id, jednostka) in ZMIENNE_BI.items():
        ...
        url = (
            ...
            f"?unit-level={aktywny_poziom}&year={rok}&format=json&page-size=100"
        )
        dane_zmiennej = []
                ...
                for miasto in MIASTA:
                    if miasto.lower() in nazwa_jednostki.lower():
                        for wartosc in wynik.get("values", []):
                            rok_danej = wartosc.get("year", rok)
    ...
    nazwa_pliku = f"baza_bi_miasta_{rok}.csv"
...
if __name__ == "__main__":
    ...
    parser.add_argument("--rok",   type=int, default=2023)
    parser.add_argument("--klucz", type=str, default=None)
    args = parser.parse_args()

    df = pobierz_dane_bi(rok=args.rok, api_key=args.klucz)
    if not df.empty:
